...
def recommendation_fixture():
    public_items = [
        {"rank": 1, "item_id": "candidate-a", "image_url": "/recommendation/images/candidate-a", "master_category": "Skirts", "coarse_category": "BOTTOM"},
        {"rank": 2, "item_id": "candidate-b", "image_url": "/recommendation/images/candidate-b", "master_category": "Jeans", "coarse_category": "BOTTOM"},
        {"rank": 3, "item_id": "candidate-c", "image_url": "/recommendation/images/candidate-c", "master_category": "Trousers", "coarse_category": "BOTTOM"},
    ]
    reranked = [
        {"item_id": "candidate-a", "compatibility_logit": 0.8, "improvement_logit": 1.2, "category_id_used": 2, "used_category_fallback": False},
        {"item_id": "candidate-b", "compatibility_logit": 0.5, "improvement_logit": 0.9, "category_id_used": 2, "used_category_fallback": False},
        {"item_id": "candidate-c", "compatibility_logit": 0.1, "improvement_logit": 0.5, "category_id_used": 2, "used_category_fallback": False},
    ]
    return {
        "status": "ok",
...
def config_fixture(*, include_raw_response=True):
    return {
        ...
        "model": {"id": "Qwen/Qwen3-VL-4B-Instruct", "dtype": "float16", "device_map": "auto", "require_cuda": True},
        "vision": {"min_pixels": 262144, "max_pixels": 262144, "image_patch_size": 16},
        "generation": {"max_new_tokens": 1024, "do_sample": False, "num_beams": 1, "repetition_penalty": 1.05, "max_validation_retries": 1},
...
class VlmPipelineV2Tests(unittest.TestCase):
    ...
    def test_handoff_is_score_free_but_preserves_ids_rank_and_visual_evidence(self):
        ...
        self.assertEqual([row["item_id"] for row in handoff["recommendations"]], ["candidate-a", "candidate-b", "candidate-c"])
        ...
        for forbidden_raw_field in (
            ...
            self.assertNotIn(forbidden_raw_field, serialized)
    ...
    def test_pipeline_fails_after_repair_budget_is_exhausted(self):
        ...
        with self.assertRaisesRegex(ValueError, "failed validation after 2 attempt"):
            pipeline.explain(evidence, outfit_images(), recommendation_images(), must_exist=False)
